...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the post-processing pipeline over a grounded answer.")
    parser.add_argument("query", type=str, help="Raw user query to answer with structured post-processing.")
    parser.add_argument(
        "--manifest-path",
        type=Path,
        default=None,
...
def main() -> None:
    ...
    grounded_answer = generate_grounded_answer(args.query, settings=settings)
    result = postprocess_grounded_answer(args.query, settings=settings, grounded_answer=grounded_answer)
